...
const matchesTagFilter = (post, tag) => {
  if (!tag) return true;
  return getPostTags(post).some((item) => item.toLowerCase() === tag.toLowerCase());
};
...
const mergeRealtimePosts = (posts, filterState) => {
  ...
  realtimePostBuffer.forEach((post, id) => {
    ...
    if (!matchesTagFilter(post, filterState.tag)) return;
...
const renderCard = (post, isNew = false) => {
  ...
  const tagsHTML = getPostTags(post).map(t => `
    <span class="card-tag" onclick='event.preventDefault(); handleTagFilter(${JSON.stringify(t)})'>#${escapeHtml(t)}</span>
  `).join('');
...
const renderFeed = async () => {
  ...
  try {
    ...
    hasMore = posts.length >= FilterState.perPage;
...
window.handleTagFilter = (tag) => {
  ...
  FilterState.activeFilter = 'latest';
  FilterState.page = 1;
  hasMore = true;
  ...
  document.querySelectorAll('.filter-pill').forEach((btn) => btn.classList.remove('active'));
  const latestPill = document.querySelector('.filter-pill[onclick*="latest"]');
  if (latestPill) latestPill.classList.add('active');
